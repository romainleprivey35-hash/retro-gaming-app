...
function showCategories() {
    ...
    view.innerHTML = `
        ...
        <div class="menu-container">
            <h1 class="main-title">${currentBrand.toUpperCase()}</h1>
            <div class="pill-menu">
...
function renderGeneralGrid(items, title) {
    const view = document.getElementById('view-list');
    view.innerHTML = `<div class="sticky-header"><button onclick="showCategories()">⬅ Retour</button></div><h2 style="text-align:center;margin-top:100px;">${title}</h2>`;
    ...
    items.forEach(g => {
        ...
        if (g.owned?.toString().toUpperCase().includes('NON')) {
            card.style.opacity = '0.35'; card.style.filter = 'grayscale(100%)';
        }
